            ...
            for idx, webhook in enumerate(config["webhooks"]):
                ...
                if "format" in webhook:
                    ...
                    if webhook["format"] not in valid_formats:
                        errors.append(
                            f"Webhook {idx} invalid format: '{webhook['format']}'. Must be one of {valid_formats}"
                        )
                ...
                if "min_level" in webhook:
                    ...
                    if not isinstance(min_level, str):
                        ...
                        errors.append(
                            f"Webhook {idx} invalid min_level: '{min_level}'. Must be one of {valid_levels}"
                        )
                ...
                if "retry_delay" in webhook:
                    ...
                    if not isinstance(value, (int, float)) or value < 0:
                        ...
                        logging.warning(
                            f"Webhook {idx} 'retry_delay' is {value}s, which is quite long. "
                ...
                if "timeout" in webhook:
                    ...
                    if not isinstance(value, (int, float)) or value <= 0:
                        ...
                        logging.warning(
                            f"Webhook {idx} 'timeout' is {value}s, which is very long. "
