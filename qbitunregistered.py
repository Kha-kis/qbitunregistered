...
parser.add_argument('--run-scheduler', action='store_true', help='If set, run the scheduler script.')
...
# Check if the --run-scheduler flag is specified and scheduler is enabled in the config
if args.run_scheduler and config.enable_scheduler:
    subprocess.run(["python", "scheduler.py"])  # Replace with the appropriate command to run scheduler.py
else:
    ...
    for torrent in client.torrents.info():
        ...
        for tracker in torrent.trackers:
            ...
            for pattern in unregistered:
                ...
                if lower_pattern.startswith("starts_with:") and lower_msg.startswith(
                        lower_pattern.split("starts_with:")[1]):
        ...
        if unregistered_count > 0:
            tags_to_add = ["unregistered:crossseeding"] if len(torrent_file_paths[torrent.save_path]) > 1 else [
                "unregistered"]
            if dry_run:
        ...
        for tracker in torrent.trackers:
            if tracker.msg != 'This torrent is private' and tracker.status == 4 and tracker.msg.lower() not in [
                p.lower() for p in unregistered]:
                tracker_short = urlsplit(tracker.url)
                logging.info("%s %s %s", torrent.name, tracker.msg, tracker_short.netloc)

                # Add a tag to the torrent
                tags_to_add = [other_issues_tag]
                if dry_run:
                    # Dry run, only print what would be done
        ...
        for tag in config.delete_tags:
            if tag in torrent.tags:
                delete_files = config.delete_files.get(tag, False)
                if not dry_run:
                    # Delete torrent and files if specified
                    client.torrents.delete(torrent.hash, delete_files=delete_files)
                    logging.info("Deleted torrent '%s' with hash %s%s.", torrent.name, torrent.hash,
                                 " and its files" if delete_files else "")
                else:
                    # Dry run, only print what would be done
                    logging.info("[Dry Run] Would delete torrent '%s' with hash %s%s.", torrent.name, torrent.hash,
                                 " and its files" if delete_files else "")
