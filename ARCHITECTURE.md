...
and no-bytecode interpreter flags; additional flags are permitted. The
coordinator subprocess and every paired child separately start with
`-B -s -S -P`, because interpreter flags do not propagate to subprocesses.
Disabling their bytecode prevents a lazy standard-library import during an
audited production call from attempting to create a temporary cache hierarchy.
Each cache prefix remains outside every evaluated repository as a defensive
containment boundary. The launcher discovers
